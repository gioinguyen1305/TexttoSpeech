...
title = "Vietnamese Voice TTS"
description = "Vietnam Voice TTS."
config_file = "config.json"
duration_model_path_female = "duration_model.pth"
duration_model_path_male = "vbx_duration_model.pth"
lightspeed_model_path_female = "gen_630k.pth"
lightspeed_model_path_male = "gen_619k.pth"
phone_set_file_female = "phone_set.json"
phone_set_file_male = "vbx_phone_set.json"
...
def text_to_phone_idx(text, gender):
    global phone_set, sil_idx
    # lowercase
    text = text.lower()
    # unicode normalize
...
def text_to_speech(duration_net, generator, text, gender):
    # prevent too long text
    if len(text) > 500:
        text = text[:500]

    phone_idx = text_to_phone_idx(text, gender)
    batch = {
...
def load_models(gender):
    duration_net = DurationNet(hps.data.vocab_size, 64, 4).to(device)
    if gender == "male":
        duration_model_path = duration_model_path_male
    else:
        duration_model_path = duration_model_path_female
    ...
    del generator.enc_q
    if gender == "male":
        lightspeed_model_path = lightspeed_model_path_male
    else:
        lightspeed_model_path = lightspeed_model_path_female
...
def load_phone_set(gender):
    global phone_set, sil_idx

    if gender == "male":
        phone_set_file = phone_set_file_male
    else:
        phone_set_file = phone_set_file_female
...
def speak(text, gender):
    load_phone_set(gender)
    duration_net, generator = load_models(gender)
    ...
    for paragraph in paragraphs:
        ...
        if paragraph == "":
            continue
        clips.append(text_to_speech(duration_net, generator, paragraph, gender))
